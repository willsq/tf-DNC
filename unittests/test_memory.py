...
class TemporalLinkAddressingTest(tf.test.TestCase):
    ...
    def test_update_link_matrix(self):
        ...
        for i, j in off_diagonal_entries:
            ...
            expected_link_matrix[:, i, j] = prev_scale * \
                prev_link_matrix[:, i, j] + write_weighting[:, i] * precedence_vector[:, j]

        new_link_matrix = TemporalLinkAddressing.update_link_matrix(
    ...
    def test_update_precedence_vector(self):
        ...
        new_precedence_vector = TemporalLinkAddressing.update_precedence_vector(
    ...
    def test_precedence_vector_behaviour_weighting_sum_one(self):
        """
...
class ContentAddressingTest(tf.test.TestCase):
    ...
    def test_weighting(self):
        ...
        memory_normalised = memory_matrix / \
            np.sqrt(np.sum(memory_matrix ** 2, axis=2, keepdims=True))
        ...
        weighting = ContentAddressing.weighting(tf.convert_to_tensor(memory_matrix),
                                                keys, strengths, sharpness_op=sharpness_op)
...
class AllocationAddressingTest(tf.test.TestCase):
    ...
    def test_update_usage_vector(self):
        ...
        expected_usage_vector = (prev_usage_vector + prev_write_weighting -
    ...
    def test_weighting_behaviour_minmax(self):
        ...
        """
        usage_vector = tf.convert_to_tensor(np.random.uniform(0, 1, (8, 13)).astype(np.float32))
        weighting = AllocationAdressing.weighting(usage_vector)

        # we require that max usage gets the min weighting
        self.assertAllEqual(tf.math.argmax(usage_vector, axis=1), tf.math.argmin(weighting, axis=1))
        # we require that min usage gets the max weighting
        self.assertAllEqual(tf.math.argmin(usage_vector, axis=1), tf.math.argmax(weighting, axis=1))
    ...
    def test_weighting_calculation(self):
        ...
        weighting = AllocationAdressing.weighting(tf.convert_to_tensor(usage_vector))
        ...
        self.assertAllClose(weighting, tf.convert_to_tensor(expected_weighting))
...
class MemoryRead(tf.test.TestCase):
    ...
    def get_addressing_weights(m, i, link_matrix):
        lookup_w = ContentAddressing.weighting(m.memory_matrix, i.read_keys, i.read_strengths)
        fwd_w, bkwd_w = TemporalLinkAddressing.weightings(link_matrix, m.read_weightings)
        read_w, read_v = Memory.read(m.memory_matrix, m.read_weightings, link_matrix, i)
    ...
    def test_read_vectors_and_weightings(self):
        ...
        for read_head in range(3):
            backward_weight = read_modes[:, 0, read_head,
                                         np.newaxis] * backward_weighting[:, :, read_head]
        ...
        expected_read_vectors = np.matmul(np.transpose(
            m.memory_matrix, [0, 2, 1]), updated_read_weightings)
